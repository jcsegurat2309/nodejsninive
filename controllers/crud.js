...
exports.register = (req, res)=>{
    ...
    conexion.query('INSERT INTO user SET ?',{name:name, email:email, password:password}, (error, results)=>{
        if(error){
            console.log(error);
        } else {
            res.redirect('/login');
...
exports.login = (req, res) => {
    ...
    conexion.query('SELECT id, email, password FROM user WHERE email = ?', [email], (err, results) => {
        if (err) {
            console.error(err);
